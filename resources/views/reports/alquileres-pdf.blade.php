        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $alquiler->usuario ? $alquiler->usuario->Nombre . ' ' . $alquiler->usuario->Apellido : 'N/A' }}</td>
                    <td>{{ $alquiler->salon ? $alquiler->salon->nombre_salon : 'N/A' }}</td>
                    <td>{{ $alquiler->cantidad_salones ?: 'N/A' }}</td>
                    <td>{{ $alquiler->fecha_alquiler ? \Carbon\Carbon::parse($alquiler->fecha_alquiler)->format('d/m/Y') : 'N/A' }}</td>
                    <td>{{ $alquiler->hora_inicio ?: 'N/A' }}</td>
                    <td>{{ $alquiler->hora_fin ?: 'N/A' }}</td>
                    <td class="{{ $alquiler->activo ? 'status-active' : 'status-inactive' }}">
                        {{ $alquiler->activo ? 'Activo' : 'Inactivo' }}
                    </td>
                    <td>{{ $alquiler->created_at ? $alquiler->created_at->format('d/m/Y H:i') : 'N/A' }}</td>
                </tr>
                @empty
                <tr>
